...
uv run pytest                 # Run 572 tests (coverage automatic)
...
│   ├── pypi_checker.py       # normalize_pypi_name(), check_pypi_availability(), validate_package_format() — async httpx
...
├── handlers/                 # 101 tests (ui_handler, filesystem, git, uv, feature_handlers)
├── ui/                       # 37 tests (dialogs)
...
- **Settings system** (`settings_manager.py`): `AppSettings` dataclass loaded from `platformdirs.user_data_dir("UV Forge")/settings.json`. Persists default project path, GitHub root, Python version, preferred IDE, git default, and default author name/email. Threaded into `AppState` at startup; settings dialog allows live editing.
...
- **Run `uv run pytest` before committing** — 588 tests, coverage automatic
