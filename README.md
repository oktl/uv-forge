...
- **Template Previewing** - Templates can be previewed and modified in the app - add or remove folders and files, same with dependency packages. The file/folder list and the packages list update automatically. Dependency packages can be marked dev and placed in the dev list in pyproject.toml. When adding a file, use the Browse button to import an existing file from disk in one step. When adding a folder, use the "Import from Disk..." button to import an entire directory tree with its file contents. You can also right-click any folder in the tree and choose "Import Folder from Disk..." to add an existing directory as a subfolder.
...
- **Presets** — Save full project configurations — including custom folders, files, and packages — as named presets for one-click reuse. Ships with 4 built-in starter presets (Flet Desktop App, FastAPI Backend, Data Science Starter, CLI Tool) for instant project setup
